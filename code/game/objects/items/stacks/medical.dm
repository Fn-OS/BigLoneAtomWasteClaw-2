...
/obj/item/stack/medical/proc/try_heal(mob/living/M, mob/user, silent = FALSE)
	...
	if(M == user)
		...
		if(!do_mob(user, M, self_delay, extra_checks=CALLBACK(M, /mob/living/proc/can_inject, user, TRUE, bypass_immunity = TRUE)))
			return
	else if(other_delay)
		if(!silent)
		...
		if(!do_mob(user, M, other_delay, extra_checks=CALLBACK(M, /mob/living/proc/can_inject, user, TRUE, bypass_immunity = TRUE)))
			return
